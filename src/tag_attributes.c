	...
	const gchar *text, GValue *value)
{
	...
	switch (value_type) {
		...
		case G_TYPE_STRING:
			...
			break;
		case G_TYPE_BOXED:
			if (paramspec->value_type != GDK_TYPE_RGBA) {
				g_value_unset(value);
				return PROPERTY_VALUE_UNSUPPORTED;
			}
			if (!gdk_rgba_parse(&color_value, text))
				goto invalid;
...
static gboolean try_set_property(GtkWidget *widget, namevalue  *nameval)
{
	...
	/* Preserve the established colorbutton XML name without touching the
	 * deprecated GtkColorButton:color GdkColor property. */
	if (GTK_IS_COLOR_CHOOSER(widget) &&
		strcmp(nameval->name, "color") == 0) {
		if (gdk_rgba_parse(&color, nameval->value)) {
			gtk_color_chooser_set_rgba(GTK_COLOR_CHOOSER(widget), &color);
			return TRUE;
		}
		if (!option_no_warning)
			gtkdialog_warning("%s(): Invalid value '%s' for property '%s'.",
				__func__, nameval->value, nameval->name);
		return FALSE;
	}
