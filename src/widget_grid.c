	...
	AttributeSet *Attr, tag_attr *attr, gint Type)
{
	...
	for (n = 0; n < children.nwidgets; ++n) {
		if ((guint)n / columns >= GRID_MAX_ATTACH) {
			...
			g_error("gtkdialog: <grid> exceeds the GTK3 attachment limit.");
		}
		...
		if (column_span > GRID_MAX_ATTACH - column) {
			gtkdialog_warning("Grid column-span exceeds the GTK3 limit; "
				"using %u.", GRID_MAX_ATTACH - column);
			column_span = GRID_MAX_ATTACH - column;
		}
		if (row_span > GRID_MAX_ATTACH - row) {
			gtkdialog_warning("Grid row-span exceeds the GTK3 limit; "
		...
		ypadding = widget_grid_take_integer(child_attr, "y-padding", 0,
			GRID_MAX_ATTACH, 0);
		gtk_widget_set_hexpand(children.widgets[n],
			(xoptions & GTK_EXPAND) != 0);
		gtk_widget_set_vexpand(children.widgets[n],
			(yoptions & GTK_EXPAND) != 0);
		gtk_widget_set_halign(children.widgets[n],
			(xoptions & GTK_FILL) != 0 ? GTK_ALIGN_FILL : GTK_ALIGN_CENTER);
		gtk_widget_set_valign(children.widgets[n],
			(yoptions & GTK_FILL) != 0 ? GTK_ALIGN_FILL : GTK_ALIGN_CENTER);
		gtk_widget_set_margin_start(children.widgets[n], xpadding);
		gtk_widget_set_margin_end(children.widgets[n], xpadding);
		gtk_widget_set_margin_top(children.widgets[n], ypadding);
		gtk_widget_set_margin_bottom(children.widgets[n], ypadding);
		gtk_grid_attach(GTK_GRID(widget), children.widgets[n], column, row,
