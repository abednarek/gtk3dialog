...
static gint widget_terminal_pixel_dimension(GtkWidget *widget,
	...
	gint *actual_cells);
static void widget_terminal_set_font_name(VteTerminal *terminal,
	const gchar *name);
#endif
	...
	AttributeSet *Attr, tag_attr *attr, gint Type)
{
	...
	if (attr) {
		...
		if ((value = get_tag_attribute(attr, "font-desc"))) {
			widget_terminal_set_font_name(VTE_TERMINAL(widget), value);
			kill_tag_attribute(attr, "font-desc");
		}

		/* VTE 2.91 removed background tinting rather than translating it
		 * to a GTK3 colour operation with different semantics. */
		if ((value = get_tag_attribute(attr, "background-tint-color"))) {
			gtkdialog_warning("VTE 2.91 does not support "
				"background-tint-color; attribute ignored.");
		...
		if ((value = get_tag_attribute(attr, "dim-foreground-color"))) {
			gtkdialog_warning("VTE 2.91 does not support "
				"dim-foreground-color; attribute ignored.");
		}
	...
	const gchar *name)
{
	...
	vte_terminal_set_font(terminal, description);
	...
	gint *actual_cells)
{
	...
	gtk_style_context_get_padding(context,
		gtk_style_context_get_state(context), &padding_border);
	padding = horizontal
	...
	if (cells > max_cells) {
		gtkdialog_warning("Terminal %s value '%d' exceeds the GTK3 widget "
