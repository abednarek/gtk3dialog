...
void on_search_entry_search_changed(GtkWidget *widget, AttributeSet *Attr)
{
	widget_signal_executor(widget, Attr, "search-changed");
}

void on_search_entry_next_match(GtkWidget *widget, AttributeSet *Attr)
{
	widget_signal_executor(widget, Attr, "next-match");
}

void on_search_entry_previous_match(GtkWidget *widget, AttributeSet *Attr)
{
	widget_signal_executor(widget, Attr, "previous-match");
}

void on_search_entry_stop_search(GtkWidget *widget, AttributeSet *Attr)
{
	widget_signal_executor(widget, Attr, "stop-search");
}
...
void on_any_widget_child_exited_event(GtkWidget *widget, gint child_status,
	AttributeSet *Attr)
{
#ifdef DEBUG_TRANSITS
	fprintf(stderr, "%s(): Entering.\n", __func__);
...
static void widget_inotify_monitor_free(gpointer data)
{
	...
	if (monitor->input_tag > 0)
		g_source_remove(monitor->input_tag);
	if (monitor->wd >= 0)
...
gboolean on_any_widget_file_changed_event(gint source,
...
#if HAVE_SYS_INOTIFY_H
gboolean on_any_widget_auto_refresh_event(gint source,
	GIOCondition condition, gpointer data)
#else
...
/* GtkWidget--->GtkContainer--->GtkBin--->GtkExpander */
			...
			} else if (GTK_IS_REVEALER(widget)) {
				is_active = gtk_revealer_get_reveal_child(GTK_REVEALER(widget));
			} else if (GTK_IS_SEARCH_BAR(widget)) {
				is_active = gtk_search_bar_get_search_mode(GTK_SEARCH_BAR(widget));
			} else if (GTK_IS_SWITCH(widget)) {
				is_active = gtk_switch_get_active(GTK_SWITCH(widget));
...
/* GtkWidget--->GtkContainer--->GtkBin--->GtkButton--->GtkToggleButton */
			if (widget_statusicon_is_proxy(widget)) {
				...
				if (GTK_IS_DIALOG(widget) &&
					strcasecmp(signal_name, "response") == 0)
					execute = widget_signal_executor_eval_condition(condition);
				else if (!GTK_IS_DIALOG(widget) &&
					(strcasecmp(signal_name, "item-activated") == 0 ||
...
/* GtkWidget--->GtkContainer--->GtkBin--->GtkButton--->GtkFontButton */
			...
			} else if (GTK_IS_FONT_CHOOSER(widget)) {
				if (strcasecmp(signal_name, "font-changed") == 0) {
					execute = widget_signal_executor_eval_condition(condition);
				}
			} else if (GTK_IS_HSV(widget)) {
...
/* GtkWidget--->GtkContainer--->GtkBin--->GtkToolItem--->GtkToolButton */
				...
				if (strcasecmp(signal_name, "file-activated") == 0)
					execute = widget_signal_executor_eval_condition(condition);
			} else if (GTK_IS_APP_CHOOSER_WIDGET(widget)) {
				if (strcasecmp(signal_name, "application-activated") == 0)
					execute = widget_signal_executor_eval_condition(condition);
/* GtkWidget--->GtkContainer--->GtkBox--->GtkHBox--->GtkFileChooserButton */
				...
				if (strcasecmp(signal_name, "response") == 0)
					execute = widget_signal_executor_eval_condition(condition);
			} else if (GTK_IS_SHORTCUTS_WINDOW(widget)) {
				if (strcasecmp(signal_name, "close") == 0)
					execute = widget_signal_executor_eval_condition(condition);
			} else if (GTK_IS_OFFSCREEN_WINDOW(widget)) {
				if (strcasecmp(signal_name, "damage-event") == 0)
					execute = widget_signal_executor_eval_condition(condition);
#ifdef GDK_WINDOWING_X11
				...
				if (strcasecmp(signal_name, "closed") == 0)
					execute = widget_signal_executor_eval_condition(condition);
/* GtkWidget--->GtkContainer--->GtkBin--->GtkExpander */
			} else if (GTK_IS_EXPANDER(widget)) {
				if (strcasecmp(signal_name, "activate") == 0) {
				...
				if (strcasecmp(signal_name, "reveal-changed") == 0)
					is_active = gtk_revealer_get_reveal_child(GTK_REVEALER(widget));
			} else if (GTK_IS_SEARCH_BAR(widget)) {
				if (strcasecmp(signal_name, "search-mode-changed") == 0)
					is_active = gtk_search_bar_get_search_mode(GTK_SEARCH_BAR(widget));
			} else if (GTK_IS_SWITCH(widget)) {
				if (strcasecmp(signal_name, "state-changed") == 0)
					is_active = gtk_switch_get_active(GTK_SWITCH(widget));
			} else if (GTK_IS_LEVEL_BAR(widget)) {
				if (strcasecmp(signal_name, "value-changed") == 0)
					execute = widget_signal_executor_eval_condition(condition);
			} else if (GTK_IS_STACK(widget)) {
				if (strcasecmp(signal_name, "page-changed") == 0)
					execute = widget_signal_executor_eval_condition(condition);
/* GtkWidget--->GtkContainer--->GtkBin--->GtkItem--->GtkMenuItem--->GtkCheckMenuItem */
			} else if (GTK_IS_CHECK_MENU_ITEM(widget)) {
				if (strcasecmp(signal_name, "toggled") == 0) {
...
/* GtkWidget--->GtkContainer--->GtkList */
			...
			} else if (GTK_IS_PLACES_SIDEBAR(widget)) {
				if (strcasecmp(signal_name, "open-location") == 0) {
					execute = widget_signal_executor_eval_condition(condition);
				}
/* GtkWidget--->GtkContainer--->GtkTreeView */
			} else if (GTK_IS_TREE_VIEW(widget)) {
				if (strcasecmp(signal_name, "row-activated") == 0 ||
					strcasecmp(signal_name, "select-row") == 0) {
					execute = widget_signal_executor_eval_condition(condition);
				}
...
/* GtkWidget--->GtkContainer--->GtkBin--->GtkExpander */
						...
						state = gtk_revealer_get_reveal_child(GTK_REVEALER(var->Widget));
					} else if (GTK_IS_SEARCH_BAR(var->Widget)) {
						state = gtk_search_bar_get_search_mode(GTK_SEARCH_BAR(var->Widget));
					} else if (GTK_IS_SWITCH(var->Widget)) {
						state = gtk_switch_get_active(GTK_SWITCH(var->Widget));
/* GtkWidget--->GtkContainer--->GtkBin--->GtkItem--->GtkMenuItem--->GtkCheckMenuItem */
				...
				if ((fd = inotify_init()) != -1) {
					...
					if ((wd = inotify_add_watch(fd, (gchar*)filename,
						...
						if (!count) {
							...
							monitor->input_tag = g_unix_fd_add(fd, G_IO_IN,
