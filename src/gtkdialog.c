		...
		char ***argv)
{
	...
	{ 
		...
		"Load styles from a GTK3 CSS file.", "filename"
	},
	{
		NULL
	}
...
"For more information try 'info gdlg3'."
);
	...
	const gchar *instance)
{
	...
	/* Compile the ordinary single-root XML fragment inside a private window. */
...
print_version_exit(int exitcode)
{
	...
	if (extralibs) printf(", ");
	printf("GtkSourceView 4"); extralibs++;
#endif
#if HAVE_VTE
	if (extralibs) printf(", ");
...
void load_styles_file(gchar *filename)
{
	...
	if (!gtk_css_provider_load_from_path(provider, filename, &error)) {
		gtkdialog_warning("Couldn't load GTK3 CSS file '%s': %s", filename,
