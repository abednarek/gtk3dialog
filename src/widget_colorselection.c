...
static void
widget_colorselection_set_value(GtkColorChooser *selection,
	const gchar *value)
{
	GdkRGBA color;
	guint alpha;
	...
	if (values->n_lines > 0 &&
		gdk_rgba_parse(&color, g_strstrip(values->line[0])))
		gtk_color_chooser_set_rgba(selection, &color);
	if (values->n_lines > 1 &&
		widget_colorselection_parse_alpha(values->line[1], &alpha)) {
		gtk_color_chooser_get_rgba(selection, &color);
...
widget_colorselection_input_by_command(variable *var, const gchar *command)
{
	...
	if (line != NULL) {
		widget_colorselection_set_value(
			GTK_COLOR_CHOOSER(var->Widget), line);
		g_free(line);
	}
...
static void
widget_colorselection_changed(GObject *object, GParamSpec *spec,
	AttributeSet *Attr)
{
	(void)spec;
	widget_signal_executor(GTK_WIDGET(object), Attr, "color-changed");
}
	...
	AttributeSet *Attr, tag_attr *attr, gint Type)
{
	...
	/* GtkColorSelection did not expose alpha unless explicitly requested. */
	gtk_color_chooser_set_use_alpha(GTK_COLOR_CHOOSER(widget), FALSE);
	if (attr != NULL) {
		value = get_tag_attribute(attr, "has-opacity-control");
		if (value != NULL) {
			gtk_color_chooser_set_use_alpha(GTK_COLOR_CHOOSER(widget),
				widget_attribute_is_true(value));
			kill_tag_attribute(attr, "has-opacity-control");
		}
		/* GtkColorChooserWidget owns its palette UI in GTK3. */
		if (get_tag_attribute(attr, "has-palette") != NULL)
			kill_tag_attribute(attr, "has-palette");
	}
	return widget;
}
...
{
	GdkRGBA color;
	GtkColorChooser *selection;

	selection = GTK_COLOR_CHOOSER(widget);
	gtk_color_chooser_get_rgba(selection, &color);
	if (gtk_color_chooser_get_use_alpha(selection))
		return g_strdup_printf("#%02x%02x%02x|%u",
			(guint)(CLAMP(color.red, 0.0, 1.0) * 255.0 + 0.5),
			(guint)(CLAMP(color.green, 0.0, 1.0) * 255.0 + 0.5),
			(guint)(CLAMP(color.blue, 0.0, 1.0) * 255.0 + 0.5),
...
void widget_colorselection_refresh(variable *var)
{
	...
	if (attributeset_is_avail(var->Attributes, ATTR_DEFAULT))
		widget_colorselection_set_value(GTK_COLOR_CHOOSER(var->Widget),
	...
	g_signal_connect(G_OBJECT(var->Widget), "notify::rgba",
