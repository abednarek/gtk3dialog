...
static gboolean widget_colorbutton_parse_alpha(const gchar *value,
	guint *result)
{
	gchar *end;
	gint64 alpha;
	...
	if (errno != 0 || end == value || *end != '\0' ||
		...
		gtkdialog_warning("Invalid colorbutton alpha value '%s'; alpha ignored.", value);
		return FALSE;
	}
...
static void widget_colorbutton_set_alpha(GtkWidget *widget, guint alpha)
{
	GdkRGBA color;
...
static guint widget_colorbutton_component(gdouble component)
{
	return (guint)(CLAMP(component, 0.0, 1.0) * 255.0 + 0.5);
}
...
gchar *widget_colorbutton_envvar_construct(GtkWidget *widget)
{
	...
	fprintf(stderr, "%s(): color.red  =#%04x  rgb=%02x\n", __func__,
		(guint)(color.red * G_MAXUINT16), widget_colorbutton_component(color.red));
	fprintf(stderr, "%s(): color.green=#%04x  rgb=%02x\n", __func__,
		(guint)(color.green * G_MAXUINT16), widget_colorbutton_component(color.green));
	fprintf(stderr, "%s(): color.blue =#%04x  rgb=%02x\n", __func__,
		(guint)(color.blue * G_MAXUINT16), widget_colorbutton_component(color.blue));
#endif

	if (gtk_color_chooser_get_use_alpha(GTK_COLOR_CHOOSER(widget))) {
		g_snprintf(envvar, sizeof(envvar), "#%02x%02x%02x|%u",
			widget_colorbutton_component(color.red),
			widget_colorbutton_component(color.green),
			widget_colorbutton_component(color.blue),
			(guint)(color.alpha * G_MAXUINT16 + 0.5));
...
void widget_colorbutton_refresh(variable *var)
{
	...
	if (!initialised) {
		...
		if (attributeset_is_avail(var->Attributes, ATTR_DEFAULT)) {
			...
			if (values->n_lines > 1) {
				if (widget_colorbutton_parse_alpha(values->line[1], &alpha)) {
#ifdef DEBUG_CONTENT
					fprintf(stderr, "%s:() valid alpha=%u found\n", __func__, alpha);
#endif
					/* This requires use-alpha="true" */
					widget_colorbutton_set_alpha(var->Widget, alpha);
				}
			}
