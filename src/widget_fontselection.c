...
static void
widget_fontselection_changed(GObject *object, GParamSpec *spec,
	gpointer data)
{
	(void)spec;
	widget_fontselection_emit_if_changed(GTK_WIDGET(object), data);
}
...
widget_fontselection_connect_changes(GtkWidget *widget, AttributeSet *Attr)
{
	...
	g_signal_connect(widget, "notify::font",
		G_CALLBACK(widget_fontselection_changed), Attr);
}
	...
	gboolean emit_signal)
{
	...
	font_name = gtk_font_chooser_get_font(GTK_FONT_CHOOSER(var->Widget));
	if (font_name == NULL)
		return TRUE;
...
{
	return gtk_font_chooser_get_font(GTK_FONT_CHOOSER(widget));
}
...
void widget_fontselection_refresh(variable *var)
{
	...
	if (var->widget_tag_attr != NULL &&
		...
		gtk_font_chooser_set_preview_text(
			GTK_FONT_CHOOSER(var->Widget), value);
