...
static void widget_edit_kill_source_buffer_attributes(tag_attr *attr)
{
...
static void widget_edit_kill_source_view_attributes(tag_attr *attr)
{
	static const gchar *names[] = {
		"show-line-numbers",
		"show-line-marks",
...
static GtkWidget *widget_edit_create_text_view(tag_attr *attr)
{
	...
	if (!widget_attribute_is_true(enabled)) {
		widget_edit_kill_source_buffer_attributes(attr);
		widget_edit_kill_source_view_attributes(attr);
		return gtk_text_view_new();
	}
	...
	gtkdialog_warning("A source editor was requested, but this build does not "
		"include GtkSourceView 4 support.");
#endif
...
void widget_edit_refresh(variable *var)
{
	...
	if (!initialised && GTK_SOURCE_IS_BUFFER(text_buffer))
