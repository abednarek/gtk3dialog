/*
 * widget_menu_json.c: Optional structured GTK3 menu input.
...
static gboolean menu_json_style_label(GtkWidget *widget, const gchar *label,
	gboolean markup, const gchar *foreground, const gchar *font,
	GError **error)
{
	...
	if (markup) {
		if (!pango_parse_markup(label, -1, 0, &attributes, &plain,
			NULL, error))
			return FALSE;
		gtk_label_set_text(GTK_LABEL(widget), plain);
		g_free(plain);
	} else {
		attributes = pango_attr_list_new();
	}
	if (foreground != NULL) {
		if (!gdk_rgba_parse(&color, foreground)) {
			g_set_error(error, G_MARKUP_ERROR, G_MARKUP_ERROR_INVALID_CONTENT,
				"invalid menu foreground '%s'", foreground);
			pango_attr_list_unref(attributes);
			return FALSE;
		}
		attribute = pango_attr_foreground_new(
			(guint16)(color.red * 65535.0 + 0.5),
			(guint16)(color.green * 65535.0 + 0.5),
			(guint16)(color.blue * 65535.0 + 0.5));
		pango_attr_list_insert(attributes, attribute);
	}
	if (font != NULL) {
		description = pango_font_description_from_string(font);
		attribute = pango_attr_font_desc_new(description);
		pango_attr_list_insert(attributes, attribute);
	...
	GError **error)
{
	...
	if (depth > JSON_MENU_MAX_DEPTH || !JSON_NODE_HOLDS_OBJECT(node)) {
		g_set_error(error, G_MARKUP_ERROR, G_MARKUP_ERROR_INVALID_CONTENT,
	...
	if (!GTK_IS_SEPARATOR_MENU_ITEM(item)) {
		...
		if (!menu_json_style_label(label_widget, label, markup, foreground,
			font, error))
			goto fail;
		if (stock != NULL || icon != NULL) {
			image = stock != NULL ?
	...
	if (action != NULL || actions_node != NULL) {
		...
		g_object_set_data_full(G_OBJECT(item),
			"gtkdialog-json-menu-actions", action_set,
	...
	GError **error)
{
	...
	for (i = 0; i < json_array_get_length(array); ++i) {
		item = menu_json_build_item(json_array_get_element(array, i), 0,
			error);
		if (item == NULL)
			goto fail;
