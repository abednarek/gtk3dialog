	...
	tag_attr *attr)
{
	...
	if (gdk_rgba_parse(&color, value)) {
		css = g_strdup_printf("drawingarea { background-color: %s; }", value);
...
static gboolean widget_drawingarea_draw(GtkWidget *widget,
	cairo_t *cr, AttributeSet *attributes)
{
	...
	if (drawing_data->pixbuf != NULL) {
		...
		if (drawing_data->scale_mode == DRAWINGAREA_SCALE_STRETCH) {
			width = MAX(1, gtk_widget_get_allocated_width(widget));
			height = MAX(1, gtk_widget_get_allocated_height(widget));
		} else if (drawing_data->scale_mode == DRAWINGAREA_SCALE_FIT) {
			ratio = MIN((gdouble)gtk_widget_get_allocated_width(widget) / width,
				(gdouble)gtk_widget_get_allocated_height(widget) / height);
			width = MAX(1, (gint)(width * ratio + 0.5));
			height = MAX(1, (gint)(height * ratio + 0.5));
			x = (gtk_widget_get_allocated_width(widget) - width) / 2;
			y = (gtk_widget_get_allocated_height(widget) - height) / 2;
		}
		display = widget_drawingarea_scaled_pixbuf(drawing_data,
			width, height);
		if (display != NULL) {
			gdk_cairo_set_source_pixbuf(cr, display, x, y);
			cairo_paint(cr);
		}
	}
...
void widget_drawingarea_refresh(variable *var)
{
	...
	g_signal_connect(G_OBJECT(var->Widget), "draw",
		G_CALLBACK(widget_drawingarea_draw), var->Attributes);
}
