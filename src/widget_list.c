...
static GtkWidget *widget_list_item_new(const gchar *value);
...
static gboolean widget_list_unselect_initial_row(gpointer user_data)
{
	gtk_list_box_unselect_all(GTK_LIST_BOX(user_data));
...
static GtkWidget *widget_list_item_new(const gchar *value)
{
	...
	g_object_set_data_full(G_OBJECT(row), "_gtkdialog-list-value",
		g_strdup(value), g_free);
	return row;
}

static void widget_list_row_selected(GtkListBox *list,
	GtkListBoxRow *row, gpointer user_data)
{
	(void)row;
	on_any_widget_selection_changed_event(GTK_WIDGET(list), user_data);
}
...
gchar *widget_list_envvar_construct(GtkWidget *widget)
{
	...
	item = gtk_list_box_get_selected_row(GTK_LIST_BOX(widget));
	if (item != NULL) {
		string = g_strdup(g_object_get_data(G_OBJECT(item),
			"_gtkdialog-list-value"));
	} else {
		string = g_strdup("");
	}
...
void widget_list_refresh(variable *var)
{
	...
	if (!initialised) {
		...
		g_signal_connect(G_OBJECT(var->Widget), "row-selected",
			G_CALLBACK(widget_list_row_selected),
		...
		if ((value = get_tag_attribute(var->widget_tag_attr, "selected-row"))) {
			...
			if (selected_row >= 0)
				gtk_list_box_select_row(GTK_LIST_BOX(var->Widget),
					gtk_list_box_get_row_at_index(GTK_LIST_BOX(var->Widget),
						selected_row));
			else
				g_idle_add_full(G_PRIORITY_DEFAULT,
					widget_list_unselect_initial_row,
					g_object_ref(var->Widget), g_object_unref);
		}
	}
...
void widget_list_removeselected(variable *var)
{
	...
	row = gtk_list_box_get_selected_row(GTK_LIST_BOX(var->Widget));
	if (row != NULL)
		...
		if ((outfile = fopen(filename, "w"))) {
			...
			while (child) {
				if (GTK_IS_LIST_BOX_ROW(child->data)) {
					string = g_object_get_data(G_OBJECT(child->data),
						"_gtkdialog-list-value");
#ifdef DEBUG_CONTENT
	...
	gint command_or_file)
{
	...
	if (infile) {
		while ((line = widget_read_line(infile)) != NULL) {
			item = widget_list_item_new(line);
...
static void widget_list_input_by_items(variable *var)
{
	...
	while (text != NULL) {
		item = widget_list_item_new(text);
