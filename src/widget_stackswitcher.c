/*
 * widget_stackswitcher.c: GTK3 GtkStackSwitcher XML widget.
...
static void widget_stackswitcher_bind(GtkWidget *widget)
{
	const gchar *target_name = g_object_get_data(G_OBJECT(widget),
		"_stackswitcher_target");
	variable *target;
	if (gtk_stack_switcher_get_stack(GTK_STACK_SWITCHER(widget)) != NULL)
		return;
	...
	if (target == NULL || target->Widget == NULL ||
		...
		gtkdialog_warning("Cannot bind <stackswitcher> to stack '%s'.",
			target_name);
		return;
	}
...
static void widget_stackswitcher_realized(GtkWidget *widget, gpointer unused)
{
	(void)unused;
	widget_stackswitcher_bind(widget);
}
	...
	gint Type)
{
	...
	if (attr == NULL ||
		...
		g_error("gtkdialog: <stackswitcher> requires a stack attribute.");
	widget = gtk_stack_switcher_new();
	g_object_set_data_full(G_OBJECT(widget), "_stackswitcher_target",
		g_strdup(target), g_free);
	kill_tag_attribute(attr, "stack");
	g_signal_connect(widget, "realize",
		G_CALLBACK(widget_stackswitcher_realized), NULL);
	return widget;
}
...
void widget_stackswitcher_refresh(variable *var)
{
	...
	if (attributeset_is_avail(var->Attributes, ATTR_DEFAULT) ||
		...
		gtkdialog_warning("<stackswitcher> has no independent value; use its <stack>.");
}
