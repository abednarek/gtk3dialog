/*
 * widget_statusicon.c: GTK3 notification-area status icon widget.
...
#define STATUS_ICON_BLINK_DATA "gtkdialog-status-icon-blink-source"
...
static gboolean widget_statusicon_blink(gpointer data)
{
	GtkStatusIcon *icon;
...
static void widget_statusicon_stop_blinking(GtkWidget *widget, gpointer data)
{
	guint source_id;

	(void)data;
	source_id = GPOINTER_TO_UINT(g_object_get_data(
		G_OBJECT(widget), STATUS_ICON_BLINK_DATA));
	if (source_id != 0) {
		g_source_remove(source_id);
		g_object_set_data(G_OBJECT(widget), STATUS_ICON_BLINK_DATA, NULL);
	}
}
	...
	AttributeSet *Attr, tag_attr *attr, gint Type)
{
	...
	if (attr != NULL) {
		...
		if ((value = get_tag_attribute(attr, "blinking")) != NULL &&
			widget_parse_boolean(value, &blinking) && blinking) {
			guint source_id = g_timeout_add(500,
				widget_statusicon_blink, widget);

			g_object_set_data(G_OBJECT(widget), STATUS_ICON_BLINK_DATA,
				GUINT_TO_POINTER(source_id));
			g_signal_connect(G_OBJECT(widget), "destroy",
				G_CALLBACK(widget_statusicon_stop_blinking), NULL);
		}
