...
widget_plug_embedded(GtkPlug *plug, gpointer user_data)
{
	 ...
	 * owned AttributeSet have been registered. Run declared actions once GTK
	 * processing begins, keeping their source order unchanged. */
	...
	AttributeSet *Attr, tag_attr *attr, gint Type)
{
	...
	/* Create the window widget. GtkPlug is an X11-only GtkWindow subclass. */
	if (Type == WIDGET_PLUG) {
#ifdef GDK_WINDOWING_X11
		Window socket_id = widget_plug_parse_socket_id(attr);
		...
		if (GDK_IS_X11_DISPLAY(gdk_display_get_default())) {
			...
			gtkdialog_warning("The <plug> root requires the GTK3 X11 backend.");
			widget = gtk_window_new(GTK_WINDOW_TOPLEVEL);
		}
#else
		if (attr != NULL && get_tag_attribute(attr, "socket-id") != NULL)
			kill_tag_attribute(attr, "socket-id");
		gtkdialog_warning("The <plug> root requires the GTK3 X11 backend.");
		widget = gtk_window_new(GTK_WINDOW_TOPLEVEL);
#endif
	...
	s = pop();
	if (program_tab_template_active() && s.nwidgets != 1) {
		gint i;

		gtkdialog_warning(program_box_template_active() ?
			"A box template must have exactly one root widget." :
			"A tab template must have exactly one root widget.");
		for (i = 0; i < s.nwidgets; ++i) {
			variables_drop_subtree(s.widgets[i]);
			gtk_widget_destroy(s.widgets[i]);
		}
	} else if (Type == WIDGET_WINDOW) {
		GtkWidget *headerbar = NULL;
		GtkWidget *content = NULL;
		gint i;
		for (i = 0; i < s.nwidgets; ++i) {
			if (GTK_IS_HEADER_BAR(s.widgets[i])) {
				if (headerbar != NULL)
					g_error("gtkdialog: <window> accepts one direct <headerbar>.");
				headerbar = s.widgets[i];
			} else if (content == NULL)
				content = s.widgets[i];
			else if (headerbar != NULL)
				gtkdialog_warning("Extra <window> content ignored after <headerbar>.");
		}
		if (headerbar != NULL) {
			const gchar *title = gtk_header_bar_get_title(
				GTK_HEADER_BAR(headerbar));
			if (title == NULL || !*title)
				gtk_header_bar_set_title(GTK_HEADER_BAR(headerbar),
					gtk_window_get_title(GTK_WINDOW(widget)));
			gtk_window_set_titlebar(GTK_WINDOW(widget), headerbar);
			if (content != NULL)
