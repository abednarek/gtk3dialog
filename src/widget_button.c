				...
				if (!input_is_shell_command(act)) {
					...
					if (stock_name != NULL) {
						...
						if (attr &&
							...
							stock_icon_size = widget_parse_stock_icon_size(value,
								GTK_ICON_SIZE_BUTTON, "button stock-icon-size");
						...
						if (attr &&
							...
							gint requested_size = widget_parse_size(value,
								"button theme-icon-size");
							if (requested_size != -1)
								theme_icon_size = requested_size;
						}
...
#ifdef DEBUG_CONTENT
							fprintf(stderr, "%s(): icon='%s' not found\n",
								__func__, icon_name);
#endif
						...
						resolved_file_name = find_pixmap(file_name);
						if (width == -1 && height == -1) {
							/* Handle unscaled images */
							icon = gtk_image_new_from_file(resolved_file_name);
						} else {
							/* Handle scaled images */
							pixbuf = gdk_pixbuf_new_from_file_at_size(
								resolved_file_name, width, height, NULL);
							if (pixbuf) {
								icon = gtk_image_new_from_pixbuf(pixbuf);
								/* pixbuf is no longer required and should be unreferenced */
								g_object_unref(pixbuf);
					...
					if (attr) {
						if ((value = get_tag_attribute(attr, "image-position"))) {
							if (strcasecmp(value, "bottom") == 0) {
								...
								position = widget_parse_bounded_integer(value,
									GTK_POS_LEFT, GTK_POS_BOTTOM, GTK_POS_LEFT,
	...
	switch (Type) {
		...
		case WIDGET_BUTTON:
			...
			if (!attributeset_is_avail(Attr, ATTR_ACTION) &&
				(attr == NULL ||
				 get_tag_attribute(attr, "response") == NULL)) {
...
static void widget_button_input_by_file(variable *var, char *filename)
{
	...
	if (attributeset_is_avail(var->Attributes, ATTR_WIDTH))
		width = widget_parse_size(attributeset_get_first(&element,
			var->Attributes, ATTR_WIDTH), "button width");
	if (attributeset_is_avail(var->Attributes, ATTR_HEIGHT))
		height = widget_parse_size(attributeset_get_first(&element,
			var->Attributes, ATTR_HEIGHT), "button height");
	...
	while (child) {
		if (GTK_IS_IMAGE(child->data)) {
			if (width == -1 && height == -1) {
				/* Handle unscaled images */
				gtk_image_set_from_file(GTK_IMAGE(child->data), resolved_filename);
			} else {
				/* Handle scaled images */
				pixbuf = gdk_pixbuf_new_from_file_at_size(
					resolved_filename, width, height, NULL);
				if (pixbuf) {
