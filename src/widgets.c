	...
	gint text_column, WidgetCompletionMatch match, gboolean case_sensitive)
{
	...
	/* Retain GTK's matching path for the established default. */
	...
	const gchar *icon_name, const gchar *image_name, gint icon_size)
{
	...
	if (stock_name != NULL && stock_name[0] != '\0') {
		pixbuf = gtk_widget_render_icon_pixbuf(widget, stock_name,
...
char *widget_get_text_value(GtkWidget *widget, int type)
{
	...
	switch (type) {
		...
		case WIDGET_STACK:
			return widget_stack_envvar_construct(widget);
		case WIDGET_STACKSWITCHER:
			return widget_stackswitcher_envvar_construct(widget);
		case WIDGET_STACKSIDEBAR:
			return widget_stacksidebar_envvar_construct(widget);
		case WIDGET_LISTBOX:
			return widget_listbox_envvar_construct(widget);
		case WIDGET_FLOWBOX:
			return widget_flowbox_envvar_construct(widget);
		case WIDGET_PLACESSIDEBAR:
			return widget_placessidebar_envvar_construct(widget);
		case WIDGET_HEADERBAR:
			return widget_headerbar_envvar_construct(widget);
		case WIDGET_POPOVER:
			return widget_popover_envvar_construct(widget);
		case WIDGET_MENUBUTTON:
			return widget_menubutton_envvar_construct(widget);
		case WIDGET_POPOVERMENU:
			return widget_popover_envvar_construct(widget);
		case WIDGET_MODELBUTTON:
			return widget_modelbutton_envvar_construct(widget);
		case WIDGET_OVERLAY:
		...
		case WIDGET_SEARCHBAR:
			return widget_searchbar_envvar_construct(widget);
		case WIDGET_ACTIONBAR:
			return widget_actionbar_envvar_construct(widget);
		case WIDGET_FILECHOOSERBUTTON:
			string = widget_filechooserbutton_envvar_construct(widget);
			return string;
			break;
		case WIDGET_APPCHOOSERBUTTON:
			return widget_appchooserbutton_envvar_construct(widget);
		case WIDGET_APPCHOOSER:
			return widget_appchooser_envvar_construct(widget);
		case WIDGET_APPCHOOSERDIALOG:
			return widget_appchooserdialog_envvar_construct(widget);
		case WIDGET_COLORCHOOSERDIALOG:
			return widget_colorchooserdialog_envvar_construct(widget);
		case WIDGET_FONTCHOOSERDIALOG:
			return widget_fontchooserdialog_envvar_construct(widget);
		case WIDGET_RECENTCHOOSERDIALOG:
			return widget_recentchooserdialog_envvar_construct(widget);
		case WIDGET_SHORTCUTSWINDOW:
		case WIDGET_SHORTCUTSSECTION:
		case WIDGET_SHORTCUTSGROUP:
		case WIDGET_SHORTCUTSSHORTCUT:
			return widget_shortcuts_envvar_construct(widget, type);
		...
		case WIDGET_STATUSICON:
			return widget_statusicon_envvar_construct(widget);
			break;
		case WIDGET_TABLE:
...
#endif
			/* A popover has no standalone toplevel surface. Its menu button
			 * shows it only after the anchor window has been realized. */
			if (entry->widget != NULL && !GTK_IS_POPOVER(entry->widget))
