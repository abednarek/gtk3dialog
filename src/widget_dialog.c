...
{
	/* GtkAboutDialog used a cancel response for its native close control in
	 * GTK2.  GTK3 reports delete-event; retain the established XML output. */
	...
	AttributeSet *Attr, tag_attr *attr, gint Type)
{
	...
	for (n = 0; n < children.nwidgets; ++n) {
		...
		if (!GTK_IS_BUTTON(children.widgets[n]) ||
			...
			gtk_widget_set_can_default(children.widgets[n], TRUE);
