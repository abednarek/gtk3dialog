	...
	AttributeSet *Attr, tag_attr *attr, gint Type)
{
	...
	if (attr) {
		/* Apply this before inserting tabs so GTK3 computes a scrollable
		 * minimum width instead of the sum of every tab label. */
		if ((value = get_tag_attribute(attr, "scrollable"))) {
			gtk_notebook_set_scrollable(GTK_NOTEBOOK(widget),
				widget_attribute_is_true(value));
			kill_tag_attribute(attr, "scrollable");
		}
	...
	if (close_enabled != NULL)
		g_object_set_data_full(G_OBJECT(widget),
			"gtkdialog-tab-close-buttons", g_strdup(close_enabled), g_free);
	if (close_action != NULL)
		g_object_set_data_full(G_OBJECT(widget),
			"gtkdialog-tab-close-action", g_strdup(close_action), g_free);
	...
	for (count = 0; count < elements.nwidgets; count++) {
		...
		label = program_notebook_tab_label(elements.widgets[count], text,
			NULL, close_enabled, close_action);
