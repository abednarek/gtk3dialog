	...
	AttributeSet *Attr, tag_attr *attr, gint Type)
{
	...
	/* Thunor: This is all original code moved across when refactoring */
	widget = Type == WIDGET_SEARCHENTRY ? gtk_search_entry_new() :
		gtk_entry_new();
	if (attr != NULL) {
...
void widget_entry_refresh(variable *var)
{
	...
	if (!initialised) {
		...
		if (GTK_IS_SEARCH_ENTRY(var->Widget)) {
			g_signal_connect(G_OBJECT(var->Widget), "search-changed",
				G_CALLBACK(on_search_entry_search_changed), var->Attributes);
			g_signal_connect(G_OBJECT(var->Widget), "next-match",
				G_CALLBACK(on_search_entry_next_match), var->Attributes);
			g_signal_connect(G_OBJECT(var->Widget), "previous-match",
				G_CALLBACK(on_search_entry_previous_match), var->Attributes);
			g_signal_connect(G_OBJECT(var->Widget), "stop-search",
				G_CALLBACK(on_search_entry_stop_search), var->Attributes);
		}
