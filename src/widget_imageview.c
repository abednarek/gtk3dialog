...
static void widget_imageview_parse_background(GtkWidget *widget,
	tag_attr *attr)
{
	GdkRGBA color;
	...
	if (gdk_rgba_parse(&color, value)) {
		...
		gtk_style_context_add_class(context, "gtkdialog-imageview");
		css = g_strdup_printf(
			".gtkdialog-imageview { background-color: %s; }", value);
	...
	ImageViewData *view_data, tag_attr *attr)
{
	...
	widget_imageview_parse_double(&view_data->zoom_step, attr, "zoom-step",
		1.01, 10.0);
	widget_imageview_parse_background(widget, attr);
	if (attr == NULL)
		return;
...
static gboolean widget_imageview_draw(GtkWidget *widget,
	cairo_t *cr, AttributeSet *attributes)
{
	ImageViewData *view_data = widget_imageview_get_data(widget);
	GdkPixbuf *rendered;
	GtkStyleContext *context;
	...
	if (view_data->pixbuf != NULL) {
		image_x = MAX(0,
			(allocated_width - view_data->displayed_width) / 2);
		image_y = MAX(0,
			(allocated_height - view_data->displayed_height) / 2);
		cairo_clip_extents(cr, &clip_x1, &clip_y1, &clip_x2, &clip_y2);
		x = MAX((gint)clip_x1, image_x);
		y = MAX((gint)clip_y1, image_y);
		width = MIN((gint)clip_x2,
			image_x + view_data->displayed_width) - x;
		height = MIN((gint)clip_y2,
		...
		if (width > 0 && height > 0) {
			...
			if (rendered != NULL) {
				...
				gdk_cairo_set_source_pixbuf(cr, rendered, x, y);
				cairo_paint(cr);
	...
	GdkEventButton *event, gpointer user_data)
{
	...
	view_data->drag_hvalue = gtk_adjustment_get_value(hadjustment);
	view_data->drag_vvalue = gtk_adjustment_get_value(vadjustment);
	return FALSE;
}
...
static GdkModifierType widget_imageview_button_mask(guint button)
{
	switch (button) {
	case 1:
...
static gdouble widget_imageview_clamp_adjustment(GtkAdjustment *adjustment,
	gdouble value)
{
	...
	GdkEventMotion *event, gpointer user_data)
{
	...
	if (!view_data->dragging)
		return FALSE;
	if ((event->state & widget_imageview_button_mask(view_data->pan_button)) == 0) {
		view_data->dragging = FALSE;
		return FALSE;
	}
	scrolled = widget_imageview_get_scrolled_window(widget);
	...
	gtk_adjustment_set_value(hadjustment,
		widget_imageview_clamp_adjustment(hadjustment, value));
	value = view_data->drag_vvalue - (event->y_root - view_data->drag_y);
	gtk_adjustment_set_value(vadjustment,
		widget_imageview_clamp_adjustment(vadjustment, value));
	return TRUE;
}
...
void widget_imageview_refresh(variable *var)
{
	...
	g_signal_connect(G_OBJECT(var->Widget), "draw",
		G_CALLBACK(widget_imageview_draw), var->Attributes);
