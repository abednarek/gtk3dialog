	...
	if (rich) {
		if (tree) {
			...
			if (Type == WIDGET_COMBOBOXENTRY) {
				widget = gtk_combo_box_new_with_model_and_entry(
	...
	variable *var, const char *name, const char *value)
{
	...
	if (widget_comboboxtext_is_rich(var->Widget))
		...
		gtk_combo_box_text_insert_text(GTK_COMBO_BOX_TEXT(var->Widget), index,
			value);
	...
	if (infile != NULL) {
		while ((line = widget_read_line(infile)) != NULL) {
			if (widget_comboboxtext_is_rich(var->Widget))
				...
				gtk_combo_box_text_append_text(GTK_COMBO_BOX_TEXT(var->Widget),
					line);
			g_free(line);
		}
		/* Close the file */
