...
#include "widget_stackswitcher.h"
...
static TabTarget *tab_target = NULL;
static GHashTable *live_tab_instances = NULL;
static gboolean fragment_attached = FALSE;

static void tab_page_accelerators_free(gpointer data)
{
	TabPageAccelerators *accelerators = data;
	GSList *element;

	if (accelerators->host != NULL) {
		if (!gtk_widget_in_destruction(accelerators->host)) {
...
static void tab_target_destroyed(GtkWidget *widget, gpointer data)
{
	(void)widget;
	((TabTarget *)data)->destroyed = TRUE;
}

gboolean program_tab_template_active(void)
{
	return tab_target != NULL;
}

gboolean program_box_template_active(void)
{
	return tab_target != NULL && tab_target->replace_box;
}

gboolean program_fragment_was_attached(void)
{
	return fragment_attached;
}

gboolean program_tab_instance_is_live(const gchar *instance)
{
	return live_tab_instances != NULL &&
		g_hash_table_lookup(live_tab_instances, instance) != NULL;
}

gboolean program_set_tab_target(GtkWidget *notebook, gint target_window_id,
	const gchar *instance, const gchar *label)
{
	if (tab_target != NULL || !GTK_IS_NOTEBOOK(notebook) ||
		program_tab_instance_is_live(instance))
		return FALSE;
	tab_target = g_new0(TabTarget, 1);
	fragment_attached = FALSE;
	tab_target->notebook = g_object_ref(notebook);
	tab_target->target_window_id = target_window_id;
	tab_target->instance = g_strdup(instance);
	tab_target->label = g_strdup(label);
	tab_target->destroy_handler = g_signal_connect(notebook, "destroy",
		G_CALLBACK(tab_target_destroyed), tab_target);
	return TRUE;
}
	...
	const gchar *instance)
{
	...
	if (tab_target != NULL || !GTK_IS_BOX(box))
		return FALSE;
	previous = live_tab_instances == NULL ? NULL :
		g_hash_table_lookup(live_tab_instances, instance);
	if (previous != NULL && gtk_widget_get_parent(previous) != box)
		return FALSE;
	tab_target = g_new0(TabTarget, 1);
	fragment_attached = FALSE;
	tab_target->notebook = g_object_ref(box);
	tab_target->target_window_id = target_window_id;
	tab_target->instance = g_strdup(instance);
	tab_target->replace_box = TRUE;
	tab_target->destroy_handler = g_signal_connect(box, "destroy",
		G_CALLBACK(tab_target_destroyed), tab_target);
	return TRUE;
}

void program_clear_tab_target(void)
{
	if (tab_target == NULL)
		return;
	if (!tab_target->destroyed && g_signal_handler_is_connected(
		tab_target->notebook, tab_target->destroy_handler))
		g_signal_handler_disconnect(tab_target->notebook,
			tab_target->destroy_handler);
	g_object_unref(tab_target->notebook);
	g_free(tab_target->instance);
	g_free(tab_target->label);
	g_free(tab_target);
	tab_target = NULL;
}

static void tab_page_destroyed(GtkWidget *page, gpointer data)
{
	...
	if (live_tab_instances != NULL && instance != NULL) {
		g_hash_table_remove(live_tab_instances, instance);
		if (g_hash_table_size(live_tab_instances) == 0) {
			g_hash_table_destroy(live_tab_instances);
			live_tab_instances = NULL;
		}
	}
}
...
{
	GtkWidget *page = live_tab_instances == NULL ? NULL :
		g_hash_table_lookup(live_tab_instances, instance);

	*source_removed = FALSE;
	if (page == NULL || !GTK_IS_NOTEBOOK(gtk_widget_get_parent(page)))
		return FALSE;
...
static void tab_close_clicked(GtkButton *button, gpointer data)
{
	...
	/* The action may remove this page and its button. Never close a newly
	 * created page that happens to reuse the same instance name. */
	g_object_ref(page);
	...
	if (instance == NULL && GTK_IS_NOTEBOOK(gtk_widget_get_parent(page))) {
		variables_drop_subtree(page);
		gtk_widget_destroy(page);
	} else if (instance != NULL && live_tab_instances != NULL &&
		g_hash_table_lookup(live_tab_instances, instance) == page)
		gtk_widget_destroy(page);
	g_object_unref(page);
	g_free(action);
	g_free(instance);
	...
	const gchar *close_action)
{
	...
	gtk_box_pack_start(GTK_BOX(tab_label), text, FALSE, FALSE, 0);
	g_object_set_data(G_OBJECT(tab_label), "gtkdialog-tab-text", text);
	...
	if (instance != NULL && close_action != NULL) {
		...
		g_strfreev(pieces);
		g_object_set_data_full(G_OBJECT(button),
			"gtkdialog-tab-close-action", expanded, g_free);
	}
	g_signal_connect(button, "clicked", G_CALLBACK(tab_close_clicked), page);
	gtk_widget_show_all(tab_label);
	return tab_label;
}

void program_notebook_set_tab_label(GtkWidget *notebook, GtkWidget *page,
	const gchar *text)
{
	GtkWidget *tab_label = gtk_notebook_get_tab_label(GTK_NOTEBOOK(notebook),
		page);
	GtkWidget *label = tab_label == NULL ? NULL :
		g_object_get_data(G_OBJECT(tab_label), "gtkdialog-tab-text");

	if (GTK_IS_LABEL(label))
		gtk_label_set_text(GTK_LABEL(label), text);
	else
		gtk_notebook_set_tab_label_text(GTK_NOTEBOOK(notebook), page, text);
}
	...
	gboolean *expand, gboolean *fill)
{
	...
	variable *box_var = find_variable_by_widget(tab_target->notebook);
	int type;
	...
	default_pack = fragment != logical ||
		...
		type == WIDGET_SCROLLEDWINDOW || type == WIDGET_HANDLEBOX ||
		type == WIDGET_TOOLPALETTE;
	if (box_var != NULL && box_var->Type == WIDGET_HBOX &&
		(type == WIDGET_ENTRY || type == WIDGET_SEARCHENTRY))
		default_pack = TRUE;
...
static GtkWidget *program_attach_tab(GtkWidget *root, gint staged_window_id)
{
	GtkWidget *page = NULL;
	GtkWidget *tab_label;
	GtkWidget *host;
	variable *root_var;
	TabPageAccelerators *accelerators;
	GSList *element;
	GList *children;
	GList *child;
	gboolean expand;
	...
	accelerators = g_new0(TabPageAccelerators, 1);
	...
	if (page == NULL || tab_target->destroyed) {
		...
		tab_page_accelerators_free(accelerators);
		return NULL;
	}
	if (tab_target->replace_box) {
		/* Stage the new tree before destroying any existing children. */
		children = gtk_container_get_children(
			GTK_CONTAINER(tab_target->notebook));
		for (child = children; child != NULL; child = child->next) {
			if (tab_target->destroyed)
				break;
	...
	variables_reassign_subtree(page, staged_window_id,
		tab_target->target_window_id);
	if (live_tab_instances == NULL)
		live_tab_instances = g_hash_table_new_full(g_str_hash, g_str_equal,
			g_free, NULL);
	g_object_set_data_full(G_OBJECT(page), "gtkdialog-tab-instance",
		g_strdup(tab_target->instance), g_free);
	host = gtk_widget_get_toplevel(tab_target->notebook);
	...
	g_object_set_data_full(G_OBJECT(page), "gtkdialog-tab-accelerators",
		accelerators, tab_page_accelerators_free);
	g_signal_connect(page, "destroy", G_CALLBACK(tab_page_destroyed), NULL);
	g_hash_table_insert(live_tab_instances, g_strdup(tab_target->instance),
		page);
	if (tab_target->replace_box) {
		program_fragment_box_packing(page, &expand, &fill);
		if (gtk_orientable_get_orientation(
			GTK_ORIENTABLE(tab_target->notebook)) ==
			GTK_ORIENTATION_HORIZONTAL)
			gtk_box_pack_end(GTK_BOX(tab_target->notebook), page,
				expand, fill, 0);
		else
			gtk_box_pack_start(GTK_BOX(tab_target->notebook), page,
				expand, fill, 0);
	} else {
		tab_label = program_notebook_tab_label(page, tab_target->label,
			tab_target->instance,
			g_object_get_data(G_OBJECT(tab_target->notebook),
				"gtkdialog-tab-close-buttons"),
			g_object_get_data(G_OBJECT(tab_target->notebook),
				"gtkdialog-tab-close-action"));
		gtk_notebook_append_page(GTK_NOTEBOOK(tab_target->notebook), page,
			tab_label);
	}
...
void print_command(instruction command)
{
	...
	switch (Widget_Type) {
		...
		case WIDGET_STACK:
			printf("(new stack())");
			break;
		case WIDGET_STACKSWITCHER:
			printf("(new stackswitcher())");
			break;
		case WIDGET_STACKSIDEBAR:
			printf("(new stacksidebar())");
			break;
		case WIDGET_LISTBOX:
			printf("(new listbox(pop()))");
			break;
		case WIDGET_FLOWBOX:
			printf("(new flowbox(pop()))");
			break;
		case WIDGET_PLACESSIDEBAR:
			printf("(new placessidebar())");
			break;
		case WIDGET_HEADERBAR:
			printf((Token & HEADERBAR_EMPTY_CHILDREN) ?
				"(new headerbar())" : "(new headerbar(pop()))");
			break;
		case WIDGET_POPOVER:
			printf("(new popover(pop()))");
			break;
		case WIDGET_MENUBUTTON:
			printf("(new menubutton(pop()))");
			break;
		case WIDGET_POPOVERMENU:
			printf("(new popovermenu(pop()))");
			break;
		case WIDGET_MODELBUTTON:
			printf("(new modelbutton())");
			break;
		case WIDGET_OVERLAY:
			printf("(new overlay(pop()))");
			break;
		case WIDGET_SWITCH:
			printf("(new switch())");
			break;
		case WIDGET_LEVELBAR:
			printf("(new levelbar())");
			break;
		case WIDGET_SEARCHBAR:
			printf("(new searchbar(pop()))");
			break;
		case WIDGET_ACTIONBAR:
			printf("(new actionbar(pop()))");
			break;
		case WIDGET_FILECHOOSERBUTTON:
			printf("(new filechooserbutton())");
			break;
		case WIDGET_APPCHOOSERBUTTON:
			printf("(new appchooserbutton())");
			break;
		case WIDGET_APPCHOOSER:
			printf("(new appchooser())");
			break;
		case WIDGET_FIXED:
		...
		case WIDGET_FILECHOOSERDIALOG:
			...
			break;
		case WIDGET_APPCHOOSERDIALOG:
			printf("(new appchooserdialog())");
			break;
		case WIDGET_COLORCHOOSERDIALOG:
			printf("(new colorchooserdialog())");
			break;
		case WIDGET_FONTCHOOSERDIALOG:
			printf("(new fontchooserdialog())");
			break;
		case WIDGET_RECENTCHOOSERDIALOG:
			printf("(new recentchooserdialog())");
			break;
		case WIDGET_SHORTCUTSWINDOW:
			printf("(new shortcutswindow(pop()))");
			break;
		case WIDGET_SHORTCUTSSECTION:
			printf("(new shortcutssection(pop()))");
			break;
		case WIDGET_SHORTCUTSGROUP:
			printf("(new shortcutsgroup(pop()))");
			break;
		case WIDGET_SHORTCUTSSHORTCUT:
			printf("(new shortcutsshortcut())");
			break;
...
void run_program()
{
	...
	gboolean          is_tab_template = tab_target != NULL;
	...
	if (is_tab_template) {
		/* Transfer the staged child before visibility processing can show
		 * the private window or connect it to the running application. */
		g_object_ref(root_widget);
		stackelement_clear(&s);
		tab_page = program_attach_tab(root_widget, window_id);
		g_object_unref(root_widget);
		widget_show_all();
		if (tab_page != NULL) {
			variables_initialize_subtree(tab_page);
			g_object_unref(tab_page);
		}
		window = NULL;
...
cleanup_program:
	...
	if (!is_tab_template && gtk_main_level() == 0) {
#ifdef DEBUG
...
void print_token(token Token)
{
	...
	switch (Widget_Type) {
		...
		case WIDGET_FLOWBOX:
			...
			break;
		case WIDGET_PLACESSIDEBAR:
			printf("(PLACESSIDEBAR)");
			break;
		case WIDGET_APPCHOOSERBUTTON:
			printf("(APPCHOOSERBUTTON)");
			break;
		case WIDGET_APPCHOOSER:
			printf("(APPCHOOSER)");
			break;
		...
		case WIDGET_FILECHOOSERDIALOG:
			...
			break;
		case WIDGET_APPCHOOSERDIALOG:
			printf("(APPCHOOSERDIALOG)");
			break;
		case WIDGET_COLORCHOOSERDIALOG:
			printf("(COLORCHOOSERDIALOG)");
			break;
		case WIDGET_FONTCHOOSERDIALOG:
			printf("(FONTCHOOSERDIALOG)");
			break;
		case WIDGET_RECENTCHOOSERDIALOG:
			printf("(RECENTCHOOSERDIALOG)");
			break;
		case WIDGET_SHORTCUTSWINDOW:
			printf("(SHORTCUTSWINDOW)");
			break;
		case WIDGET_SHORTCUTSSECTION:
			printf("(SHORTCUTSSECTION)");
			break;
		case WIDGET_SHORTCUTSGROUP:
			printf("(SHORTCUTSGROUP)");
			break;
		case WIDGET_SHORTCUTSSHORTCUT:
			printf("(SHORTCUTSSHORTCUT)");
			break;
		case WIDGET_PAGESETUPDIALOG:
...
gvim_position_window(GtkWidget *placeholder)
{
	...
	window = gtk_widget_get_window(placeholder);
	if (popup == NULL || !gtk_widget_get_realized(placeholder) ||
		window == NULL)
		return;

	gdk_window_get_origin(window, &x, &y);
	gtk_widget_get_allocation(placeholder, &allocation);
...
create_gvim(AttributeSet * Attr)
{
	...
	Window socket_id_value;
	char socket_id[32];

	gtk_widget_set_events(fake, GDK_ALL_EVENTS_MASK);
	gtk_widget_set_size_request(popup_window, 350, 150);
	gtk_widget_set_size_request(fake, 350, 150);
	
	gtk_container_add(GTK_CONTAINER(popup_window), socket);
	...
	g_object_set_data(G_OBJECT(fake), GVIM_WINDOW_DATA, popup_window);
	g_signal_connect(fake, "realize",
		G_CALLBACK(gvim_placeholder_realized), popup_window);
	g_signal_connect(fake, "unrealize",
		G_CALLBACK(gvim_placeholder_unrealized), popup_window);
	g_signal_connect(fake, "destroy",
		G_CALLBACK(gvim_placeholder_destroyed), popup_window);
	return fake;
#else
	gtkdialog_warning("The <gvim> widget requires the GTK3 X11 backend.");
		...
		tag_attr      *tag_attributes)
{
	...
	switch (Widget_Type) {
		...
		case WIDGET_REVEALER:
			...
			break;
		case WIDGET_SEARCHBAR:
			Widget = widget_searchbar_create(Attr, tag_attributes, Widget_Type);
			push_widget(Widget, Widget_Type);
			lastradiowidget = NULL;
			break;
		case WIDGET_ACTIONBAR:
			Widget = widget_actionbar_create(Attr, tag_attributes, Widget_Type);
			push_widget(Widget, Widget_Type);
			lastradiowidget = NULL;
			break;
		case WIDGET_STACK:
			...
			break;
		case WIDGET_STACKSWITCHER:
			Widget = widget_stackswitcher_create(Attr, tag_attributes,
				Widget_Type);
			push_widget(Widget, Widget_Type);
			break;
		case WIDGET_STACKSIDEBAR:
			Widget = widget_stacksidebar_create(Attr, tag_attributes,
				Widget_Type);
			push_widget(Widget, Widget_Type);
			break;
		case WIDGET_LISTBOX:
			Widget = widget_listbox_create(Attr, tag_attributes, Widget_Type);
			push_widget(Widget, Widget_Type);
			lastradiowidget = NULL;
			break;
		case WIDGET_FLOWBOX:
			...
			break;
		case WIDGET_PLACESSIDEBAR:
			Widget = widget_placessidebar_create(Attr, tag_attributes,
				Widget_Type);
			push_widget(Widget, Widget_Type);
			break;
		case WIDGET_HEADERBAR:
			Widget = widget_headerbar_create(Attr, tag_attributes,
				Token & WIDGET_SUBTYPE);
			push_widget(Widget, Widget_Type);
			lastradiowidget = NULL;
			break;
		case WIDGET_POPOVER:
			Widget = widget_popover_create(Attr, tag_attributes, Widget_Type);
			push_widget(Widget, Widget_Type);
			lastradiowidget = NULL;
			break;
		case WIDGET_MENUBUTTON:
			Widget = widget_menubutton_create(Attr, tag_attributes, Widget_Type);
			push_widget(Widget, Widget_Type);
			lastradiowidget = NULL;
			break;
		case WIDGET_POPOVERMENU:
			Widget = widget_popovermenu_create(Attr, tag_attributes, Widget_Type);
			push_widget(Widget, Widget_Type);
			lastradiowidget = NULL;
			break;
		case WIDGET_MODELBUTTON:
			Widget = widget_modelbutton_create(Attr, tag_attributes, Widget_Type);
			push_widget(Widget, Widget_Type);
			break;
		case WIDGET_OVERLAY:
		...
		case WIDGET_FILECHOOSERBUTTON:
			...
			break;
		case WIDGET_APPCHOOSERBUTTON:
			Widget = widget_appchooserbutton_create(
				Attr, tag_attributes, Widget_Type);
			push_widget(Widget, Widget_Type);
			break;
		case WIDGET_APPCHOOSER:
			Widget = widget_appchooser_create(
				Attr, tag_attributes, Widget_Type);
			push_widget(Widget, Widget_Type);
			break;
		case WIDGET_FIXED:
		...
		case WIDGET_STATUSBAR:
			...
			break;
		case WIDGET_STATUSICON:
			Widget = widget_statusicon_create(Attr, tag_attributes, Widget_Type);
			push_widget(Widget, Widget_Type);
			break;
		case WIDGET_TABLE:
		...
		case WIDGET_FILECHOOSERDIALOG:
			...
			break;
		case WIDGET_APPCHOOSERDIALOG:
			Widget = window = widget_appchooserdialog_create(
				Attr, tag_attributes, Widget_Type);
			push_widget(Widget, Widget_Type);
			break;
		case WIDGET_COLORCHOOSERDIALOG:
			Widget = window = widget_colorchooserdialog_create(
				Attr, tag_attributes, Widget_Type);
			push_widget(Widget, Widget_Type);
			break;
		case WIDGET_FONTCHOOSERDIALOG:
			Widget = window = widget_fontchooserdialog_create(
				Attr, tag_attributes, Widget_Type);
			push_widget(Widget, Widget_Type);
			break;
		case WIDGET_RECENTCHOOSERDIALOG:
			Widget = window = widget_recentchooserdialog_create(
				Attr, tag_attributes, Widget_Type);
			push_widget(Widget, Widget_Type);
			break;
		case WIDGET_SHORTCUTSWINDOW:
			Widget = window = widget_shortcuts_create(
				Attr, tag_attributes, Widget_Type);
			push_widget(Widget, Widget_Type);
			break;
		case WIDGET_SHORTCUTSSECTION:
		case WIDGET_SHORTCUTSGROUP:
		case WIDGET_SHORTCUTSSHORTCUT:
			Widget = widget_shortcuts_create(
				Attr, tag_attributes, Widget_Type);
			push_widget(Widget, Widget_Type);
			break;
		case WIDGET_PAGESETUPDIALOG:
