...
void widget_scrolledwindow_clear(variable *var)
{
	(void)var;
...
GtkWidget *widget_scrolledwindow_create(
	AttributeSet *Attr, tag_attr *attr, gint Type)
{
	...
	if (child.nwidgets != 1) {
		...
		g_error("gtkdialog: <scrolledwindow> requires exactly one direct "
			"child widget.");
	}
...
gchar *widget_scrolledwindow_envvar_construct(GtkWidget *widget)
{
	(void)widget;
	return g_strdup("");
}

void widget_scrolledwindow_fileselect(
...
void widget_scrolledwindow_refresh(variable *var)
{
	GList *element;
	...
	if (attributeset_is_avail(var->Attributes, ATTR_WIDTH))
		width = widget_parse_size_request(attributeset_get_first(&element,
			var->Attributes, ATTR_WIDTH), "scrolled window width");
	if (attributeset_is_avail(var->Attributes, ATTR_HEIGHT))
		height = widget_parse_size_request(attributeset_get_first(&element,
			var->Attributes, ATTR_HEIGHT), "scrolled window height");
	if (width != -1 || height != -1)
...
void widget_scrolledwindow_removeselected(variable *var)
{
	(void)var;
	fprintf(stderr,
...
void widget_scrolledwindow_save(variable *var)
{
	(void)var;
