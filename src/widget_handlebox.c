	...
	AttributeSet *Attr, tag_attr *attr, gint Type)
{
	...
	/* GTK3 deprecated GtkHandleBox without providing a supported detachable
	 * replacement. Keep the XML container and its child as a static box. */
	widget = gtk_box_new(GTK_ORIENTATION_VERTICAL, 0);
	gtk_container_add(GTK_CONTAINER(widget), child.widgets[0]);
	stackelement_clear(&child);
	if (attr != NULL) {
		kill_tag_attribute(attr, "handle-position");
		kill_tag_attribute(attr, "snap-edge");
		kill_tag_attribute(attr, "shadow-type");
	}
	return widget;
}
