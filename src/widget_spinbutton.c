...
static void widget_spinbutton_validate_range(gdouble *minimum,
	gdouble *maximum, gdouble *step)
{
	if (*minimum >= *maximum || !isfinite(*maximum - *minimum)) {
		gtkdialog_warning("Invalid spinbutton range; using 0..100.");
		*minimum = 0.0;
		*maximum = 100.0;
	}
	if (*step <= 0.0) {
		gtkdialog_warning("Invalid spinbutton range-step; using 1.");
		*step = 1.0;
	}
}
	...
	AttributeSet *Attr, tag_attr *attr, gint Type)
{
	...
	if (attr) {
		if ((value = get_tag_attribute(attr, "range-min")))
			widget_parse_finite_double(value, &range_min,
				"spinbutton range-min");
		if ((value = get_tag_attribute(attr, "range-max")))
			widget_parse_finite_double(value, &range_max,
				"spinbutton range-max");
		if ((value = get_tag_attribute(attr, "range-step")))
			widget_parse_finite_double(value, &range_step,
				"spinbutton range-step");
		if ((value = get_tag_attribute(attr, "range-value")))
			widget_parse_finite_double(value, &range_value,
				"spinbutton range-value");
	}
	widget_spinbutton_validate_range(&range_min, &range_max, &range_step);
...
gchar *widget_spinbutton_envvar_construct(GtkWidget *widget)
{
	...
	if (digits > 16)
		digits = 6;
	string = g_strdup_printf("%.*f", digits, value);
	...
	if (infile = widget_opencommand(command)) {
		...
		if ((line = widget_read_line(infile))) {
			...
			if (widget_parse_finite_double(line, &value,
				"spinbutton command input"))
				gtk_spin_button_set_value(GTK_SPIN_BUTTON(var->Widget), value);
			g_free(line);
		}
		/* Close the file */
