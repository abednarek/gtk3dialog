...
#define TREE_FILTER_DATA "gtkdialog-tree-filter-data"
...
static void widget_tree_set_progress_orientation(GtkCellRenderer *renderer,
	const gchar *value)
{
	...
	if (strcasecmp(value, "right-to-left") == 0)
		inverted = TRUE;
	else if (strcasecmp(value, "bottom-to-top") == 0) {
		orientation = GTK_ORIENTATION_VERTICAL;
		inverted = TRUE;
	} else if (strcasecmp(value, "top-to-bottom") == 0) {
		orientation = GTK_ORIENTATION_VERTICAL;
	} else if (strcasecmp(value, "left-to-right") != 0) {
		gtkdialog_warning("Invalid tree column-progress-orientation value '%s'; "
			"using left-to-right.", value);
	}
	g_object_set(renderer, "orientation", orientation,
...
static GtkTreeModel *widget_tree_source_model(GtkWidget *tree_view)
{
...
static gboolean widget_tree_source_iter(GtkWidget *tree_view,
	GtkTreeIter *view_iter, GtkTreeIter *source_iter)
{
	TreeFilterData *filter = g_object_get_data(G_OBJECT(tree_view),
		TREE_FILTER_DATA);
	GtkTreeIter filter_iter;
	if (filter == NULL || filter->model == NULL) {
		*source_iter = *view_iter;
		return TRUE;
	}
	gtk_tree_model_sort_convert_iter_to_child_iter(GTK_TREE_MODEL_SORT(
		gtk_tree_view_get_model(GTK_TREE_VIEW(tree_view))), &filter_iter,
		view_iter);
	gtk_tree_model_filter_convert_iter_to_child_iter(filter->model,
		source_iter, &filter_iter);
	return TRUE;
}

static gboolean widget_tree_source_iter_from_string(GtkWidget *tree_view,
	const gchar *path, GtkTreeIter *source_iter,
	GtkTreeIter *view_iter)
{
	GtkTreeIter current;
	GtkTreeModel *model = gtk_tree_view_get_model(GTK_TREE_VIEW(tree_view));
	if (!gtk_tree_model_get_iter_from_string(model, &current, path))
		return FALSE;
	if (view_iter != NULL)
		*view_iter = current;
	return widget_tree_source_iter(tree_view, &current, source_iter);
}

static GtkTreePath *widget_tree_source_path(GtkWidget *tree_view,
	GtkTreePath *view_path)
{
	...
	filter_path = gtk_tree_model_sort_convert_path_to_child_path(
		GTK_TREE_MODEL_SORT(gtk_tree_view_get_model(GTK_TREE_VIEW(tree_view))),
		view_path);
	if (filter_path == NULL)
		return NULL;
	source_path = gtk_tree_model_filter_convert_path_to_child_path(
		filter->model, filter_path);
	gtk_tree_path_free(filter_path);
	return source_path;
}

static GtkTreePath *widget_tree_view_path(GtkWidget *tree_view,
	GtkTreePath *source_path)
{
	TreeFilterData *filter = g_object_get_data(G_OBJECT(tree_view),
		TREE_FILTER_DATA);
	...
	GtkTreePath *view_path;
	if (filter == NULL || filter->model == NULL)
		return gtk_tree_path_copy(source_path);
	filter_path = gtk_tree_model_filter_convert_child_path_to_path(
		filter->model, source_path);
	if (filter_path == NULL)
		return NULL;
	view_path = gtk_tree_model_sort_convert_child_path_to_path(
		GTK_TREE_MODEL_SORT(gtk_tree_view_get_model(GTK_TREE_VIEW(tree_view))),
		filter_path);
	gtk_tree_path_free(filter_path);
	return view_path;
...
static gboolean widget_tree_filter_row_matches(GtkTreeModel *model,
	GtkTreeIter *iter, const gchar *query)
{
	gint column;
	for (column = FirstDataColumn;
		column < gtk_tree_model_get_n_columns(model); ++column) {
		gchar *text = widget_tree_model_string(model, iter, column);
		gchar *folded = g_utf8_casefold(text != NULL ? text : "", -1);
		gboolean matched = strstr(folded, query) != NULL;
		g_free(folded);
		g_free(text);
		if (matched)
			return TRUE;
...
static gboolean widget_tree_filter_descendant_matches(GtkTreeModel *model,
	GtkTreeIter *iter, const gchar *query, guint depth)
{
	GtkTreeIter child;
	if (depth >= TREE_JSON_MAX_DEPTH ||
		...
		return FALSE;
	do {
		if (widget_tree_filter_row_matches(model, &child, query) ||
			widget_tree_filter_descendant_matches(model, &child, query,
				depth + 1))
			return TRUE;
...
static gboolean widget_tree_filter_visible(GtkTreeModel *model,
	GtkTreeIter *iter, gpointer user_data)
{
	TreeFilterData *filter = user_data;
	GtkTreeIter current = *iter;
	GtkTreeIter parent;
	if (filter->query == NULL || *filter->query == '\0')
		return TRUE;
	if (widget_tree_filter_row_matches(model, iter, filter->query) ||
		widget_tree_filter_descendant_matches(model, iter, filter->query, 0))
		return TRUE;
	while (gtk_tree_model_iter_parent(model, &parent, &current)) {
		if (widget_tree_filter_row_matches(model, &parent, filter->query))
			return TRUE;
		current = parent;
	}
	return FALSE;
...
static gboolean widget_tree_filter_path_is_expanded(TreeFilterData *filter,
	GtkTreePath *source_path)
{
	guint index;
	for (index = 0; index < filter->expanded->len; ++index) {
		GtkTreePath *path = gtk_tree_row_reference_get_path(
			g_ptr_array_index(filter->expanded, index));
		gboolean equal = path != NULL &&
			gtk_tree_path_compare(path, source_path) == 0;
		if (path != NULL)
...
static void widget_tree_filter_row_expanded(GtkTreeView *view,
	GtkTreeIter *iter, GtkTreePath *path, TreeFilterData *filter)
{
	GtkTreePath *source_path;
	GtkTreeRowReference *reference;
	(void)iter;
	if (filter->refiltering)
		return;
	source_path = widget_tree_source_path(GTK_WIDGET(view), path);
	if (source_path == NULL)
		return;
	if (!widget_tree_filter_path_is_expanded(filter, source_path)) {
		reference = gtk_tree_row_reference_new(
			widget_tree_source_model(GTK_WIDGET(view)), source_path);
		if (reference != NULL)
			g_ptr_array_add(filter->expanded, reference);
	}
	gtk_tree_path_free(source_path);
}

static void widget_tree_filter_row_collapsed(GtkTreeView *view,
	GtkTreeIter *iter, GtkTreePath *path, TreeFilterData *filter)
{
	GtkTreePath *source_path;
	guint index;
	...
	if (source_path == NULL)
		return;
	for (index = 0; index < filter->expanded->len;) {
		GtkTreePath *current = gtk_tree_row_reference_get_path(
			g_ptr_array_index(filter->expanded, index));
		gboolean contained = current != NULL &&
			(gtk_tree_path_compare(current, source_path) == 0 ||
			gtk_tree_path_is_ancestor(source_path, current));
		if (current != NULL)
			gtk_tree_path_free(current);
		if (contained)
			g_ptr_array_remove_index(filter->expanded, index);
		else
			++index;
	}
	gtk_tree_path_free(source_path);
...
static void widget_tree_filter_data_free(gpointer data)
{
	TreeFilterData *filter = data;
	g_ptr_array_free(filter->expanded, TRUE);
	g_free(filter->entry_name);
	g_free(filter->query);
	g_free(filter);
}

static void widget_tree_filter_refilter(GtkWidget *tree_view,
	TreeFilterData *filter)
{
	if (filter->model == NULL)
		return;
	filter->refiltering = TRUE;
	gtk_tree_model_filter_refilter(filter->model);
	{
		guint index;
		for (index = 0; index < filter->expanded->len; ++index) {
			GtkTreePath *source_path = gtk_tree_row_reference_get_path(
				g_ptr_array_index(filter->expanded, index));
			GtkTreePath *view_path = source_path != NULL
				? widget_tree_view_path(tree_view, source_path) : NULL;
			if (view_path != NULL) {
	...
	filter->refiltering = FALSE;
}

static void widget_tree_filter_entry_changed(GtkEntry *entry,
	GtkWidget *tree_view)
{
	TreeFilterData *filter = g_object_get_data(G_OBJECT(tree_view),
		TREE_FILTER_DATA);
	...
	widget_tree_filter_refilter(tree_view, filter);
}

static void widget_tree_filter_bind(GtkWidget *tree_view, gpointer unused)
{
	TreeFilterData *filter = g_object_get_data(G_OBJECT(tree_view),
		TREE_FILTER_DATA);
	variable *entry;
	...
	if (entry == NULL || !GTK_IS_ENTRY(entry->Widget)) {
		if (gtk_widget_get_realized(tree_view))
			gtkdialog_warning("Cannot bind <tree> filter to entry '%s'.",
				filter->entry_name);
		return;
	}
	filter->bound = TRUE;
	g_signal_connect_object(entry->Widget, "changed",
		G_CALLBACK(widget_tree_filter_entry_changed), tree_view, 0);
	widget_tree_filter_entry_changed(GTK_ENTRY(entry->Widget), tree_view);
}

static void widget_tree_filter_deactivate(GtkWidget *tree_view)
{
	TreeFilterData *filter = g_object_get_data(G_OBJECT(tree_view),
		TREE_FILTER_DATA);
	GtkTreeModel *source;
	if (filter == NULL || filter->model == NULL)
		return;
	source = widget_tree_source_model(tree_view);
	g_object_ref(source);
	filter->refiltering = TRUE;
	filter->model = NULL;
	gtk_tree_view_set_model(GTK_TREE_VIEW(tree_view), source);
	filter->refiltering = FALSE;
	g_object_unref(source);
}

static void widget_tree_filter_activate(GtkWidget *tree_view,
	tag_attr *attr)
{
	TreeFilterData *filter = g_object_get_data(G_OBJECT(tree_view),
		TREE_FILTER_DATA);
	...
	if (filter == NULL || filter->model != NULL)
		return;
	source = gtk_tree_view_get_model(GTK_TREE_VIEW(tree_view));
	filter_model = gtk_tree_model_filter_new(source, NULL);
	filter->model = GTK_TREE_MODEL_FILTER(filter_model);
	gtk_tree_model_filter_set_visible_func(filter->model,
		widget_tree_filter_visible, filter, NULL);
	sort_model = gtk_tree_model_sort_new_with_model(filter_model);
	sort_values = attr != NULL ? get_tag_attribute(attr,
		"column-sort-function") : NULL;
	functions = sort_values != NULL ? g_strsplit(sort_values, "|", -1) : NULL;
	for (column = FirstDataColumn;
		functions != NULL &&
		functions[column - FirstDataColumn] != NULL &&
		column < gtk_tree_model_get_n_columns(sort_model);
		++column) {
		gint function = widget_parse_bounded_integer(
			functions[column - FirstDataColumn], 0, 2, 0,
			"tree column-sort-function");
		if (gtk_tree_model_get_column_type(sort_model, column) !=
			G_TYPE_STRING)
			continue;
		if (function == 1 || function == 2)
			gtk_tree_sortable_set_sort_func(
				GTK_TREE_SORTABLE(sort_model), column,
				function == 1 ? widget_tree_natcmp :
					widget_tree_natcasecmp,
	...
	const gchar *value)
{
	...
	if (pixbuf == NULL)
		return NULL;
	rgba = ((guint32)(color.red * 255.0 + 0.5) << 24) |
		((guint32)(color.green * 255.0 + 0.5) << 16) |
		((guint32)(color.blue * 255.0 + 0.5) << 8) | 0xff;
	...
	TreeImageSource source, const gchar *value, gint size)
{
	...
	if (source == TreeImageIconName) {
		...
		loaded = gtk_widget_render_icon_pixbuf(data->tree_view, value,
			widget_tree_image_stock_size(size));
		if (loaded == NULL)
...
static void widget_tree_renderer_style_free(gpointer user_data)
{
	...
	if (style->background != NULL)
		gdk_rgba_free(style->background);
	if (style->foreground != NULL)
		gdk_rgba_free(style->foreground);
	...
	GtkCellRenderer *renderer, gint column)
{
	...
	g_object_get(G_OBJECT(renderer),
		...
		"cell-background-rgba", &style->background,
		"xalign", &style->xalign,
		"yalign", &style->yalign,
		"xpad", &style->xpad,
	...
	TreeRendererStyle *style)
{
	...
	if (style->foreground_set)
		g_object_set(G_OBJECT(renderer), "foreground-rgba",
	...
	gchar *path, gchar *new_text, TreeCellData *data)
{
	...
	(void)renderer;
	model = widget_tree_source_model(data->tree_view);
	if (!widget_tree_source_iter_from_string(data->tree_view, path,
		&iter, NULL))
		return;
...
static void widget_tree_enable_completion(GtkCellRenderer *renderer,
	...
	gint minimum_key_length,
	WidgetCompletionMatch match, gboolean case_sensitive)
{
	...
	gchar *path, TreeCellData *data)
{
	...
	model = widget_tree_source_model(tree_view);
	if (!widget_tree_source_iter_from_string(tree_view, path,
		&iter, NULL))
		goto done;
	if (!widget_tree_json_cell_sensitive(model, &iter, data->model_column))
		goto done;
	source_path = gtk_tree_model_get_path(model, &iter);

	if (data->radio) {
		if (widget_tree_get_column_boolean(model, &iter,
			data->model_column)) {
			gtk_tree_path_free(source_path);
			goto done;
		}
	...
	selection = gtk_tree_view_get_selection(GTK_TREE_VIEW(tree_view));
	view_model = gtk_tree_view_get_model(GTK_TREE_VIEW(tree_view));
	view_path = widget_tree_view_path(tree_view, source_path);
	if (view_path != NULL &&
		gtk_tree_model_get_iter(view_model, &view_iter, view_path) &&
		gtk_tree_selection_get_mode(selection) != GTK_SELECTION_NONE &&
		!gtk_tree_selection_iter_is_selected(selection, &view_iter))
		gtk_tree_selection_select_iter(selection, &view_iter);
	if (view_path != NULL)
		gtk_tree_path_free(view_path);
	gtk_tree_path_free(source_path);
...
void widget_tree_clear(variable *var)
{
	...
	model = widget_tree_source_model(var->Widget);
	widget_tree_renderer_caches_clear(var->Widget);
	if (g_object_get_data(G_OBJECT(var->Widget), TREE_FILTER_DATA) != NULL) {
		TreeFilterData *filter = g_object_get_data(G_OBJECT(var->Widget),
			TREE_FILTER_DATA);
		filter->refiltering = TRUE;
		g_ptr_array_set_size(filter->expanded, 0);
	}
	...
	if (g_object_get_data(G_OBJECT(var->Widget), TREE_FILTER_DATA) != NULL &&
		((TreeFilterData *)g_object_get_data(G_OBJECT(var->Widget),
			TREE_FILTER_DATA))->model != NULL)
	...
	AttributeSet *Attr, tag_attr *attr, gint Type)
{
	...
	if (attr != NULL &&
		...
		if (*value != '\0') {
			TreeFilterData *filter = g_new0(TreeFilterData, 1);
			filter->entry_name = g_strdup(value);
			filter->expanded = g_ptr_array_new_with_free_func(
				(GDestroyNotify)gtk_tree_row_reference_free);
			g_object_set_data_full(G_OBJECT(widget), TREE_FILTER_DATA,
				filter, widget_tree_filter_data_free);
			g_signal_connect(widget, "realize",
				G_CALLBACK(widget_tree_filter_bind), NULL);
			g_signal_connect(widget, "row-expanded",
				G_CALLBACK(widget_tree_filter_row_expanded), filter);
			g_signal_connect(widget, "row-collapsed",
				G_CALLBACK(widget_tree_filter_row_collapsed), filter);
		} else {
			gtkdialog_warning("<tree> filter-entry needs an entry name.");
		}
...
void widget_tree_refresh(variable *var)
{
	...
	gboolean          restored_json_state = FALSE;
	...
	widget_tree_filter_deactivate(var->Widget);
		...
		if (completion_input) {
			if (completion_column >= 0) {
				...
				if (completion_store == NULL) {
					...
					widget_tree_input_completion(completion_store, act + 8,
						TRUE);
	...
	widget_tree_filter_activate(var->Widget, var->widget_tag_attr);
	...
	if (!initialised) {
		...
		if (g_object_get_data(G_OBJECT(var->Widget), TREE_FILTER_DATA) != NULL)
			widget_tree_filter_bind(var->Widget, NULL);
	}
	...
	if (var->widget_tag_attr) {
		...
		/* Resolve stable JSON IDs after loading, sorting and filtering. */
#if HAVE_JSON_GLIB
		if (preserved != NULL) {
			restored_json_state = widget_tree_restore_state(preserved, var);
			widget_tree_preserved_state_free(preserved);
			preserved = NULL;
		}
#endif
		/* Get selected-row (custom) */
		if (!restored_json_state &&
...
void widget_tree_removeselected(variable *var)
{
	...
	selection = gtk_tree_view_get_selection(GTK_TREE_VIEW(var->Widget));
	selectionmode = gtk_tree_selection_get_mode(selection);
	source_model = widget_tree_source_model(var->Widget);
	if (selectionmode == GTK_SELECTION_NONE) {
		/* Nothing to do */
	} else if (selectionmode == GTK_SELECTION_MULTIPLE) {
		...
		if (gtk_tree_selection_count_selected_rows(selection)) {
			...
			row = selectedrows;
			while (row) {
				GtkTreePath *source_path = widget_tree_source_path(
					var->Widget, row->data);
				if (source_path != NULL) {
					rowreferences = g_list_prepend(rowreferences,
						gtk_tree_row_reference_new(source_model, source_path));
					gtk_tree_path_free(source_path);
				}
				row = row->next;
			}
			...
			while (row) {
				...
				if (path != NULL &&
					gtk_tree_model_get_iter(source_model, &iter, path))
					gtk_tree_store_remove(GTK_TREE_STORE(source_model), &iter);
				if (path != NULL)
					gtk_tree_path_free(path);
		...
		if (gtk_tree_selection_get_selected(selection, &model, &iter)) {
			...
			widget_tree_source_iter(var->Widget, &iter, &source_iter);
			gtk_tree_store_remove(GTK_TREE_STORE(source_model),
				&source_iter);
		}
	}
	if (selectionmode != GTK_SELECTION_NONE)
	...
	tag_attr *attr, GtkTreeStore *store)
{
	...
	for (index = 0; index < columns->n_lines; ++index) {
		...
		if (column_is_color) {
			...
			if (value != NULL)
				widget_tree_set_progress_orientation(renderer, value);
			value = (gchar *)widget_tree_column_value(
		...
		if (column_has_completion) {
			if (!column_is_editable) {
				...
				widget_tree_enable_completion(renderer, tree_view, index,
					completion_store, function,
					completion_match, completion_case_sensitive);
...
static gboolean widget_tree_capture_source_row(GtkTreeModel *model,
	GtkTreePath *path, GtkTreeIter *iter, gpointer user_data)
{
	...
	if (g_hash_table_lookup_extended(state->rows, id, NULL, NULL)) {
		/* Duplicate IDs cannot safely refer to either row. */
...
static gboolean widget_tree_capture_visible_row(GtkTreeModel *model,
	GtkTreePath *path, GtkTreeIter *iter, gpointer user_data)
{
	TreePreservedState *state = user_data;
	TreePreservedRow *row;
	...
	id = widget_tree_json_row_id(model, iter);
	row = id != NULL ? g_hash_table_lookup(state->rows, id) : NULL;
	if (row != NULL) {
		selection = gtk_tree_view_get_selection(
			GTK_TREE_VIEW(state->tree_view));
		row->selected = gtk_tree_selection_path_is_selected(selection, path);
		row->expanded = gtk_tree_view_row_expanded(
			GTK_TREE_VIEW(state->tree_view), path);
...
static TreePreservedState *widget_tree_preserve_state(GtkWidget *tree_view)
{
	...
	source = widget_tree_source_model(tree_view);
	view = gtk_tree_view_get_model(GTK_TREE_VIEW(tree_view));
	gtk_tree_model_foreach(source, widget_tree_capture_source_row, state);
	gtk_tree_model_foreach(view, widget_tree_capture_visible_row, state);
	filter = g_object_get_data(G_OBJECT(tree_view), TREE_FILTER_DATA);
	if (filter != NULL) {
		for (index = 0; index < filter->expanded->len; ++index) {
			GtkTreePath *path = gtk_tree_row_reference_get_path(
				g_ptr_array_index(filter->expanded, index));
			GtkTreeIter iter;
			if (path != NULL && gtk_tree_model_get_iter(source, &iter, path)) {
				gchar *id = widget_tree_json_row_id(source, &iter);
				TreePreservedRow *row = id != NULL ?
					g_hash_table_lookup(state->rows, id) : NULL;
				if (row != NULL)
					row->expanded = TRUE;
				g_free(id);
			}
...
static gboolean widget_tree_index_source_row(GtkTreeModel *model,
	GtkTreePath *path, GtkTreeIter *iter, gpointer user_data)
{
	GHashTable *paths = user_data;
	gchar *id = widget_tree_json_row_id(model, iter);
	if (id == NULL)
		return FALSE;
	...
	variable *var)
{
	...
	source = widget_tree_source_model(state->tree_view);
	filter = g_object_get_data(G_OBJECT(state->tree_view), TREE_FILTER_DATA);
	gtk_tree_model_foreach(source, widget_tree_index_source_row, paths);
	if (filter != NULL) {
		guint index = 0;
		while (index < filter->expanded->len) {
			GtkTreePath *path = gtk_tree_row_reference_get_path(
				g_ptr_array_index(filter->expanded, index));
			GtkTreeIter iter;
			gchar *id = NULL;
			...
			if (path != NULL && gtk_tree_model_get_iter(source, &iter, path)) {
				id = widget_tree_json_row_id(source, &iter);
				replace = id != NULL &&
					g_hash_table_lookup(state->rows, id) != NULL &&
					g_hash_table_lookup(paths, id) != NULL;
			}
			g_free(id);
			if (path != NULL)
				gtk_tree_path_free(path);
	...
	while (g_hash_table_iter_next(&rows, &key, &value)) {
		TreePreservedRow *row = value;
		GtkTreePath *source_path = g_hash_table_lookup(paths, key);
		GtkTreePath *view_path;
		if (row == NULL || source_path == NULL)
			continue;
		matched = TRUE;
		if (row->expanded && filter != NULL) {
			GtkTreeRowReference *reference =
				gtk_tree_row_reference_new(source, source_path);
			if (reference != NULL)
				g_ptr_array_add(filter->expanded, reference);
		}
		view_path = widget_tree_view_path(state->tree_view, source_path);
		if (view_path == NULL)
			continue;
		if (row->expanded)
			gtk_tree_view_expand_to_path(GTK_TREE_VIEW(state->tree_view),
				view_path);
		if (row->selected)
			selected = g_list_prepend(selected, gtk_tree_path_copy(view_path));
		if (row->anchor)
			anchor = gtk_tree_path_copy(view_path);
		gtk_tree_path_free(view_path);
	}
	/* Expanding a child may expand its parent, so collapse parents last. */
	g_hash_table_iter_init(&rows, state->rows);
	while (g_hash_table_iter_next(&rows, &key, &value)) {
		TreePreservedRow *row = value;
		GtkTreePath *source_path = g_hash_table_lookup(paths, key);
		GtkTreePath *view_path;
		if (row == NULL || row->expanded || source_path == NULL)
			continue;
		view_path = widget_tree_view_path(state->tree_view, source_path);
		if (view_path != NULL) {
			gtk_tree_view_collapse_row(GTK_TREE_VIEW(state->tree_view),
				view_path);
			gtk_tree_path_free(view_path);
		}
	}
	...
	GtkTreeModel *model, guint depth, GError **error)
{
	...
	for (index = 0; index < json_array_get_length(rows); ++index) {
		...
		if (member != NULL && (!JSON_NODE_HOLDS_VALUE(member) ||
			...
			json_node_get_string(member)[0] == '\0')) {
			g_set_error(error, G_MARKUP_ERROR, G_MARKUP_ERROR_INVALID_CONTENT,
				"tree JSON id in row %u at depth %u is not a non-empty string",
				index, depth);
			return FALSE;
		}
		...
		if (!member || !JSON_NODE_HOLDS_VALUE(member) ||
			...
			if (!member || !JSON_NODE_HOLDS_ARRAY(member)) {
				...
				if (widget_tree_json_validate_rows(rows,
					widget_tree_source_model(var->Widget), 1,
					&error)) {
					...
					for (row = expanded_rows; row; row = row->next) {
						expanded_path = gtk_tree_row_reference_get_path(
							row->data);
						if (expanded_path) {
							GtkTreePath *view_path = widget_tree_view_path(
								var->Widget, expanded_path);
							if (view_path != NULL) {
								gtk_tree_view_expand_to_path(
									GTK_TREE_VIEW(var->Widget), view_path);
								gtk_tree_path_free(view_path);
							}
	...
	GtkTreeView *tree_view, GtkTreeIter *parent)
{
	...
	model = widget_tree_source_model(GTK_WIDGET(tree_view));
	...
	while (valid) {
		...
		if (g_object_get_data(G_OBJECT(tree_view), TREE_FILTER_DATA) != NULL) {
			TreeFilterData *filter = g_object_get_data(G_OBJECT(tree_view),
				TREE_FILTER_DATA);
			json_builder_add_boolean_value(builder, path != NULL &&
				widget_tree_filter_path_is_expanded(filter, path));
		} else {
			json_builder_add_boolean_value(builder, path != NULL &&
